...
RealForce::RealForce() {
    _absError = {0.147494444,-12.51203333,-36.31094444,0.954612167,0.615341056,0.110116389};
...
void RealForce::setRobotName(const std::string &strRobotName) {
    ...
    _ikSolver = ikFactory->createKinematicSolver(strRobotName);

    boost::shared_ptr<RobotDriverFactoryInterface> rdFactory = RobotDriverFactoryInterface::create();
    _robotDriver = rdFactory->createRobotDriver(strRobotName);

    _robotDriver->connect("192.168.10.10");
        ...
        const std::vector<double>& forceFromSensor,const std::vector<double >& currentPose)
{
    ...
    double theta = 0.0/180.0*constPI;
